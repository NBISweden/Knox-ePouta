...
if (!params.partition) {
    exit 1, 'You need to specify a slurm partition, see --help for more information'
}
...
// Try to guess location of bamindex file. If we cannot find it create it
...
// Try to guess location of fastq file. If we cannot find it create it
...
// channel. We cannot use toSortedList here since the full pathname is used
...
process snpEff {
    ...
    if [ -z "\$SNPEFFJAR" ]; then
        printf "Cannot find snpEff.jar in '%s'" "\$CLASSPATH" >&2
        exit 1
    fi
...
def usage_message() {
    ...
    log.info '    --parition      Slurm partition to use'
    log.info '  Optional'
...
def startup_message() {
    ...
    log.info "Partition  : $params.partition"
    log.info "Will run   : " + workflowSteps.join(", ")
    log.info ""
}

def grab_git_revision() {
    if ( workflow.commitId ) { // it is run directly from github
