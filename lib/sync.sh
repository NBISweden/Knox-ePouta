...
do
	...
	( # In a subshell
	    ...
	    python -c "import os, sys, jinja2; \
		   ...
		   <${FOLDER}/sync.jn2 \
		   >${_SCRIPT}
	    ssh -t -F ${SSH_CONFIG} ${FLOATING_IPs[$machine]} 'sudo bash -e -x 2>&1' <${_SCRIPT}
	)
	RET=$?
...
EOF
    ...
    ssh -t -F ${SSH_CONFIG} ${FLOATING_IPs[supernode]} 'sudo bash -e -x 2>&1' <<EOF &>/dev/null
...
EOF
	...
	ssh -t -F ${SSH_CONFIG} ${FLOATING_IPs[$machine]} 'sudo bash -e -x 2>&1' <<EOF &>/dev/null
