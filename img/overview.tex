...
\begin{tikzpicture}
  ...
  \path[name path=border] (link) -- ++(100:65mm) node[right=6ex,pos=0.5](fin){Finland} node[left=4ex,pos=0.4](swe){Sweden};
  \path[name path=limit] (vm1-epouta) -- (vm1-knox);
  \path[name intersections={of=border and limit, by=t}] (link) -- (t);

  %% Extend a bit
  \path (link) coordinate(b);
  \path (b) ++(right:45mm) coordinate(b-r) ++(0,5) coordinate(t-r);
  \path (b) ++(left:60mm)  coordinate(b-l) ++(0,5) coordinate(t-l);
 
  \begin{pgfonlayer}{my background}
    %\draw[red] (b-r) -- (t-r) -- (t-l) -- (b-l);
    \clip (b-r) -- (t-r) -- (t-l) -- (b-l);

    %% Swedish shade
    \shade[right color=blue!20,left color=white]%
    (b) decorate[decoration={snake,segment length=6ex, amplitude=0.5ex}]{ -- (t) }
    decorate[decoration=lineto] { -- (t-l) -- (b-l) -- cycle };

    %% Finnish shade
    \shade[left color=yellow!50,right color=white]%
    (b) decorate[decoration={snake,segment length=6ex, amplitude=0.5ex}]{ -- (t) }
