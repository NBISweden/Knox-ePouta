...
\item the set of VMs on ePouta range from \ip{10.101.0.4} to \ip{10.101.127.255},% and
\item the set of VMs on Knox range from \ip{10.101.128.1} to \ip{10.101.128.254}.
\end{itemize}
...
\footnote{%
  ...
  non-optimal): \codeline{bridge-ageing.sh}
  %
...
\codeline{gw~{\veth}~mm}), along with another \emph{local} network
...
% \codeline{\$ source ePouta.credentials>}
% \codeline{\$ neutron subnet-show epouta-subnet}
\codeblock{epouta.sh}
