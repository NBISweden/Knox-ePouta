...
[ $VERBOSE == 'no' ] && exec 1>${KE_TMP}/provision.log
ORG_FD1=$(tty)
...
do
    ...
    _TEMPLATE=${PROFILES}/${MACHINE_PROFILES[$machine]}/provision.jn2
    if [ ! -f ${_TEMPLATE} ]; then
	oups "\tProvisioning script unknown for $machine"
	filter_out_machine $machine
    else

	_SCRIPT=${KE_TMP}/$machine/provision/run.sh
	_LOG=${KE_TMP}/$machine/provision/log
...
if (( FAIL > 0 )); then
    oups "\a\n${FAIL} servers failed to be provisioned"
else
    # Finally...
    thumb_up "\nServers provisioned"
fi
