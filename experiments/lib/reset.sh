...
[ $VERBOSE == 'no' ] && exec 1>${KE_TMP}/reset.log
ORG_FD1=$(tty)
...
#######################################################################
source $(dirname ${BASH_SOURCE[0]})/lib/utils.sh reset
...
do
    ...
    _TEMPLATE=${PROFILES}/${MACHINE_PROFILES[$machine]}/reset.jn2
    if [ ! -f ${_TEMPLATE} ]; then
	oups "\tResetting script unknown for $machine"
	filter_out_machine $machine
    else

	_SCRIPT=${KE_TMP}/$machine/reset/run.sh
	_LOG=${KE_TMP}/$machine/reset/log
	...
	{ # Scoping, in that current shell
	    ssh -F ${SSH_CONFIG} ${MACHINE_IPs[$machine]} 'sudo bash -e -x 2>&1' <${_SCRIPT} &>${_LOG}
	    RET=$?
...
for job in ${JOB_PIDS[@]}; do wait $job || ((FAIL++)); 
